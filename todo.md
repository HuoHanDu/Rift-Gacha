...
- [x] **P6.1 详情弹层（触屏主交互）**
  - 原问题：资料卡是纯 CSS `:hover` 浮层，**固定向上开**，靠近视口顶部必被裁；而纯 CSS 无法感知视口。
    给浮层加定位计算能修，但**小程序没有 `getBoundingClientRect`**（要用 `uni.createSelectorQuery()`）。
  - 方案：改成**底部弹层**——不需要任何定位数学，窄屏贴底、宽屏居中，H5 与小程序都能跑。
    用模块级单例状态（`src/components/detailSheet.ts`）+ 页面里唯一的 `DetailSheet.vue`，
    `IconChip` 只写入 payload。**不做平台判断**：桌面保留悬停浮层，同时也能点击开弹层。
  - **完成判据**：114 用例通过（新增 2 个：弹层渲染、共享状态契约「同一时刻只有一个弹层」）；`build:h5` 与 `build:mp-weixin` 都通过，小程序产物里有 `DetailSheet.wxml`。✅

- [x] **P6.2 窄屏布局与触控目标**
  - ≤380px 时成装一行允许换行（原为 `nowrap`：34×7 + 6×6 + 9 ≈ 283px，320px 屏卡内只有约 260px，会横向溢出），并隐藏那个竖线分隔符。
  - 用 `@media (hover: none)`（而不是宽度断点——平板也是触屏）放大可点区域：`.seg__item` / `.btn` 接近 44px，玩家名输入框高 38px。
  - 弹层底部加 `env(safe-area-inset-bottom)`，适配刘海屏。
  - **完成判据**：两平台构建通过。⚠️ 实际观感需要在真机/窄屏浏览器里看。

- [ ] **P6.3 未做**（按需再说）
  - 单位仍是 `px` + media query，没换成 `rpx`；小程序真机未验证。
  - 小程序 `manifest.json` 的 `mp-weixin.appid` 仍为空。
...
| 2026-09-22 | P0–P5 完成并部署到 `rift.huohandu.cn`；v1 优化五则（去 `descHtml`、资料卡对齐、IME、键盘可达性、符文详情） |
