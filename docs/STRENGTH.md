...
| 5 | 权重 | 胜率 : 登场率 = **7 : 3**；绝对量级 **k1 = 7.0、k2 = 3.0**（即 `(胜率−50)×7 + 登场率×3`）|
| 6 | 分路 T 挡位 | **T0 +27 / T1 +19 / T2 +11 / T3 +4 / T4 +0**；**没有该分路数据 = 0 分**（不扣分）。**T 挡位贡献必须 ≈ 胜率/登场率的 80%，不能压过它**——小代/主播会把胜率打高（如打野豹女），实测数据比官方分档更可信 |
| 7 | 装备适配度 | 命中推荐 +10/+5；不在推荐里则按适配度判相容（见 §5.1）。**相容分具体给多少暂缓**——当前测得的装备期望只是下界，等分类表做完量一次再定（见 §10）|
| 8 | 符文 | 主系基石命中 **+7**；主系 3 个小符文各 **+4**；副系 2 个各 **+2**；3 个属性碎片各 **+2**（重合才加）。满分 29。**N = 5**（见 §9）|
| 9 | 实时调整 | **目标分 + 容差，反复重试直到落进区间**。默认容差 ±10%，界面可调 |
| 10 | 数据版本 | 装备/符文快照升到 **16.19**；**101 胜率数据实测只能用 16.18**（16.19 未铺数据），落后如实标注 |
| 11 | **目标比例** | **英雄 : 装备 : 符文 ≈ 6 : 5 : 3**（按**期望值**对齐，不是按满分）|
...
## 9. 装备分类表（2026-09-23 完成）

`items.json` 新增 `categories` 字段：`装备ID → ["ad","ap","crit","attackSpeed","tank","support"]`。
一件装备**可以属多类**（界弓 = 物理/法强/攻速），比强行归一准确。
...
### 一个必须记住的教训：**辅助类不能用标签启发式**

第一版用 `types` 标签判辅助，实测误判率很高：

| 误判 | 驱动标签 | 真相 |
| --- | --- | --- |
| 日炎圣盾 / 冰霜之心 → 辅助 | `Aura` | 纯**坦克**装，只是带伤害/减攻速光环 |
| 夺萃之镰 → 辅助 | `ManaRegen` | "夺萃"是回蓝被动，它是**物理暴击**装 |
| 斯塔缇克电刃 → 法强 | `SpellDamage` | 它不给法强，只是普攻附带魔法伤害 |
| 无穷饥渴 → 法强/坦度 | `SpellVamp` / `Tenacity` | 两个标签都不代表这两类 |

所以最终方案：

- **辅助**改成**显式名单**（`SPEC.supportByName`，11 件）。辅助装本来就是明确的小集合，用名单比用标签准得多。
- **法强**去掉 `SpellVamp`（无穷饥渴带这个标签但是纯物理吸血装）。
- **坦度**去掉 `Tenacity`（有韧性的多半是输出装，如水银弯刀、无穷饥渴）。
- **坦度**保留一条有原则的补充规则：**有血量、且完全没有输出标签**（`offenseTags` 全不命中）
  ⇒ 坦度装。这条救回了凛冬之临（`types=["AbilityHaste","Health","Mana"]`，
  既无护甲也无伤害标签，但语义上就是坦克/战士的蓝量装）。
- **`Health` 单独出现不算坦度**：46 件装备带 Health，包括三相之力和兰德里的折磨这类输出装。
- **人工覆盖**（`SPEC.itemCategoryOverrides`）优先级最高，且是**替换**而不是叠加。
- 结构性断言：每件传说装备必须至少归入一类，否则 `fetch:data` **构建失败**
  （这条断言当场抓出了凛冬之临）。

---

## 10. 定案数据（2026-09-23 模拟）

`node scripts/analyze-total-score.mjs 20000` —— 20000 次蒙特卡洛，按生成器真实结构抽样，
用的是真实 101 数据（239 个英雄×分路组合）。
...
**实际比例 5.96 : 5.21 : 2.83**（目标 6 : 5 : 3，各项误差 < 5%）。

### 装备相容分定案：**强相容 +2 / 弱相容 +1**

原提案是 +4/+1，实测**装备项会爆到 20.90（占 7.45/14）**。根因是一个必须记住的数字：

> **91.4% 的成装抽取都不在推荐列表里。** 其中强相容 47.5%、弱相容 37.9%、不相容 14.6%。
> 所以相容分给多少会被放大 6 倍，+4 太高了。

反推后定 **+2 / +1**，装备项落到 15.69，比例就对上了。
...
1. **右偏且拖尾很长**：中位数 41，但上限 128（T0 英雄 + 满命中构筑 + 满命中符文）。
2. **负分可达**（p0 = −28.5），用户已确认允许——"那很区了"。
3. 分布很宽（极差 156），所以挡位区间**不能等宽切**。
...
> 上界 128 会让「类人」显得过宽。若要三档"手感"均衡，可能该把「类人」下界抬到 60 左右，
> 并把超出部分当作"超模"。这一条由用户定。
